...
public class PaidOrderTests
{
    ...
    [Fact]
    public async Task Given_WhenCallDeliver_ThenDeliveredOrderReturned()
    {
        ...
        DeliveredOrder? actualResult = null;
        
        Func<Task> action = async () => actualResult = await order.Deliver(); 
        
        using (new AssertionScope())
        {
            await action.Should().NotThrowAsync();
            actualResult.Should().NotBeNull();
            actualResult.Should().BeOfType<DeliveredOrder>();
    ...
    [Fact]
    public async Task Given_WhenCallRefund_ThenRefundedOrderReturned()
    {
        var address = new Address
        {
            Id = this.fixture.Create<string>(),
        ...
        RefundedOrder? actualResult = null;
        
        Func<Task> action = async () => actualResult = await order.Refund(); 
        
        using (new AssertionScope())
        {
            await action.Should().NotThrowAsync();
            actualResult.Should().NotBeNull();
            actualResult.Should().BeOfType<RefundedOrder>();
            actualResult.Should().BeAssignableTo<IOrder>();
            actualResult?.Id.Should().Be(order.Id);
            actualResult?.CreatedAt.Should().Be(order.CreatedAt);
            actualResult?.UpdatedAt.Should().BeAfter(order.UpdatedAt);
