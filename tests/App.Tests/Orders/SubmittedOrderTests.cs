...
public class SubmittedOrderTests
{
    ...
    [Fact]
    public async Task Given_WhenCallCancel_ThenCanceledOrderReturned()
    {
        ...
        CanceledOrder? actualResult = null;
        
        Func<Task> action = async () => actualResult = await order.Cancel(); 
        
        using (new AssertionScope())
        {
            await action.Should().NotThrowAsync();
            actualResult.Should().NotBeNull();
            actualResult.Should().BeOfType<CanceledOrder>();
    ...
    [Fact]
    public async Task Given_WhenCallPay_ThenPaidOrderReturned()
    {
        var address = new Address
        {
            Id = this.fixture.Create<string>(),
        ...
        PaidOrder? actualResult = null;
        
        Func<Task> action = async () => actualResult = await order.Pay(); 
        
        using (new AssertionScope())
        {
            await action.Should().NotThrowAsync();
            actualResult.Should().NotBeNull();
            actualResult.Should().BeOfType<PaidOrder>();
            actualResult.Should().BeAssignableTo<IOrder>();
            actualResult?.Id.Should().Be(order.Id);
            actualResult?.CreatedAt.Should().Be(order.CreatedAt);
            actualResult?.UpdatedAt.Should().BeAfter(order.UpdatedAt);
